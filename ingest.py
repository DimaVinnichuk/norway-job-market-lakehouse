...
def get_pipeline_status(get_date):
    ...
    if STATUS_FILE_PATH.exists():
        logger.info("Pipeline status file found at %s. Reading existing state.", STATUS_FILE_PATH)
        content = STATUS_FILE_PATH.read_text(encoding="utf-8").strip()
        logger.debug("Retrieved date from status file: %s", content)
        return content
    else:
        logger.info("Pipeline status file NOT found at %s. Triggering historical backfill mode.", STATUS_FILE_PATH)
...
def fetch_and_save_vacancy_data(token, time_point):
    """Fetch and save vacancy data"""

    header_data = {
        "Authorization": f"Bearer {token}",
        "If-Modified-Since": f"{time_point}"
    }
    logger.info("Fetching vacancy data from API...")
    ...
    while page_url:
        ...
        if data_json.get("items"):
            ...
            logger.debug("Retrieved page saved to %s.", path_to_data)
            page_number+=1
            if next_url:
                page_url = urljoin(DOMAIN, next_url)
            else:
    ...
    STATUS_FILE_PATH.write_text(timestamp_for_update_state, encoding="utf-8")
...
def run_ingestion():
    ...
    defined_datetime_point = get_pipeline_status(generate_history_point)
    jwt_token = get_jwt_token()
    fetch_and_save_vacancy_data(jwt_token, defined_datetime_point)
